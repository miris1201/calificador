...
window.handleCiudadanos = (id) => {

    $('#id_modal_detalle').modal('show');
    const url = `business/oficialia/remisiones/ajax/dataNmCiudadanos.php?id=${ id }`;
    fetch(url)
        .then((resp) => resp.text())
        .then(function(resp) {
            sel('#table_ondata').innerHTML = resp;
        })
        .catch(function(error) {
            Swal.fire({
                icon: 'error',
